...
@router.get(
    "/{analysis_id}/stream",
    summary="Stream live execution trace"
)
async def stream_trace_endpoint(
    analysis_id: str,
    token: str | None = Query(None, description="Auth token (required since EventSource doesn't support headers)")
) -> StreamingResponse:
    """
    ...
    """
    if not token:
        raise AuthenticationError("Not authenticated", detail="Token must be passed as query parameter")
