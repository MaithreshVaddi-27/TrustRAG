...
### 🏆 Option 2A: Render.com (Recommended — 100% Free, Zero Setup Friction)
...
4. Render automatically detects `render.yaml` and displays a configuration form:
...
5. Click **Apply**.
6. Render builds the Docker container and assigns your live HTTPS URL:
   ```
...
1. Click **New +** → **Web Service** → Connect your `TrustRAG` repository.
...
4. Under **Health Check Path**, enter: `/api/v1/health`.
...
3. Click **Add Variables** and insert:
   - `APP_ENV` = `production`
   - `MONGODB_DATABASE` = `trustrag_db`
   - `JWT_SECRET` = *(your 64-char secret)*
   - `GEMINI_API_KEY` = *(your Gemini key)*
...
2. Select your repository: `TrustRAG`.
...
4. Expand **Environment variables (production)**:
   - Add variable:
     ...
     - **Value**: `https://trustrag-api.onrender.com` *(or your Railway / Cloud Run live backend URL)*
